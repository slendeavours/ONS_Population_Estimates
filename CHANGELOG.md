...
### Added
- **S6 refreshed to the year-ending-June-2026 edition.** `la_asylum_support` now holds 34 quarters to 2026-06-30 (21,953 rows, 2,245,677 people); `la_immigration_groups` holds 7,104 rows across two retained snapshots. All 13 checks pass.
- **S11 refreshed to the 01 September 2026 CQC directory.** 30,561 active locations upserted, 108 deactivated as absent from the new file, 30,797 rows total. 296/296 authorities covered; 30,793 of 30,797 resolved by point-in-polygon, five locations unmappable and logged open in `cqc_unresolved_locations`.
- **`docs/decisions/2026-09-04-s6-reg02-hardcoded-snapshot-period.md`** — the root cause below, and the two lessons worth carrying: one constant was doing two jobs, and a test can be passing *because* of the defect rather than in spite of it.

### Fixed
- **Reg_02's snapshot period was fixed in source, so every S6 refresh overwrote the previous snapshot in place.** `parse_reg_02` set `period = ANCHOR_PERIOD`, a literal 2026-03-31. Reg_02 is one snapshot per edition, so the June 2026 edition was written onto the March rows' primary key. **No structural test was sensitive to it**: the row count would have stayed at 3,552, every key unique, all 296 authorities present, and Reg_02's internal pathway-against-total reconciliation passing, because June reconciles against itself perfectly well. Check 9 — the cross-source comparison against Asy_D11 — failed with 263 of 286 authorities divergent and rolled the load back. The period now comes from the edition label via `_edition_period()`, which hard-stops on a label it cannot parse rather than assuming a date. **A cross-source gate earns its keep on the second edition, not the first.**
- **Check 4 was passing because of that defect, not in spite of it.** It counted `la_immigration_groups` unfiltered and compared the total against the rows just parsed — an assertion that only held while every load overwrote in place. It now counts at the snapshot period being loaded and reports how many snapshots are retained.
- **`ANCHOR_PERIOD` was doing two jobs and only one was wrong.** It is a verification anchor for checks 3, 5, 8a and 8b — a period whose published headline figures were sourced by hand — *and* it was Reg_02's data period. The anchor legitimately stays at 2026-03-31: Asy_D11 is a full time series, each edition still contains the anchored period, and an unchanged anchor that still reconciles is evidence the back series was not revised. It did reconcile exactly. The refresh procedure in `docs/s6_asylum_source.md` now separates the two and states the limit plainly — **a quarter loaded without re-anchoring has not had its England and UK headline totals compared against anything the Home Office published in prose.**
- **`_write_anomalies` had been writing to `scripts/docs/` since the 2026-08-20 tidy**, leaving the tracked `docs/s6_source_anomalies.md` frozen at the 26 July run. It built its path from the script's own directory, which was correct while the script sat at the repository root. Same defect class as the `.env` resolution gate 11 covers, and **gate 11 does not cover it — it asserts how scripts resolve credentials, not how they resolve output paths.** Nothing checks the latter.

### Changed
- `docs/s6_asylum_source.md` updated to the current state: edition, 34-quarter range, per-table row and people counts, and the row-count reconciliation. Reg_02's shape is now recorded as "snapshot per edition, retained" rather than "single snapshot".
...
### Added
- **`docs/decisions/README.md`** — an index reconciling the decision records, saying which findings are closed and which are still open. Each record is left as written, because a record quietly edited later is no longer evidence of anything; the index is the reconciliation. Three things are still open: the seven-quarter S1 A1 revision, the S10 marker check, and the 129 historical stored zeros.
- **`scripts/push.py` — the push is now structurally gated and cannot proceed on a failing suite.** The order (scan, verify, then push) had failed twice by being remembered rather than enforced: once the scan ran after the push, once the suite ran in the same command as the push and its exit code was read after the irreversible step. The check and the action now live in one place and cannot be separated. Seven steps: clean working tree (a dirty tree means the suite verified a state that is not the one being pushed), commits to be pushed, credential scan over added lines only, verification suite, generated-artefact staleness, push, then confirm the remote head matches the local head. Exit 1 blocks; exit 2 (known-red only) is allowed, which is the suite's own contract.
- **`--install-hook` installs a pre-push hook**, so a bare `git push` is gated too and the control does not depend on typing the right command. The hook skips re-running the suite when invoked through `push.py`.
- **Gate 11 now covers 36 scripts, up from 18, and stopped matching `os.environ`.** Testing for `.env` as a bare substring also matched `dict(os.environ, ...)`, which is an environment-variable read and not a dotenv file — it flagged `push.py` and `credential_scan.py` on their first run. The test is now the filename in the forms it is actually written, plus any script importing `_db`, so a script that delegates stays asserted instead of dropping out of scope. Broader, not narrower; proved by injection with a script resolving `.env` from the repo root only.
- **`scripts/credential_scan.py`** — the scan as a script rather than something retyped from memory at each push. A control reconstructed by hand every time has a different definition every time. Nine patterns, a narrow explicit allowlist, and `--range` scans only the lines a push would add.
- **`sql/source_registry.sql` carries the writer-only trigger**, so the protection is reproducible from the versioned schema rather than existing only in the live database.
- **`docs/GENERATED_FIELDS.md`** — which registry fields are generated and from where, so anyone editing knows which columns are theirs. Established as a data test, not a code read: the regex read got it wrong, reporting `revision_note` as hand-maintained when the backfill demonstrably writes it. Snapshot the table, perturb every column on every row, run the backfill once, see what comes back; snapshot restored and verified exact afterwards.
- **`source_registry` is now writer-only at the destination.** A trigger rejects any INSERT or UPDATE from a connection that has not declared itself with `SET ucws.registry_writer = 'on'`; `backfill_source_registry.py` and `check_sources.py` declare themselves, nothing else may write. Previously a hand edit to a *declared* cell reverted on the next backfill (gate 9 caught one), but a hand edit to an **undeclared** cell persisted silently until somebody added a declaration for that source — it works until it doesn't, and nothing says when it stopped. 12 columns are regenerated for every source, 27 more only for the sources declaring them, 4 are unmanaged.
- **Gate 17 — the writer-only trigger is present and enabled.** A control nobody checks is a control that has already gone. Proved by injection: disabling the trigger fails the gate and exits 1.
- **A known-red entry whose gate currently passes is now a hard failure**, as is an entry for a gate the suite does not define. Either the entry is stale or the gate is being masked, and neither may be silent. Proved by injection: both cases exit 1 with a named reason.

### Fixed
- **Gate 14 green; the suite exits 0 with `known_red.json` empty.** The two 2018 S.114 notices against `E10000021` (Northamptonshire County Council, abolished 2021) are retained against the issuing code and deliberately not propagated. The premise was checked rather than asserted: statutory homelessness and housing register cover 296/296 authorities while care leavers covers 132/296, so housing is a lower-tier function and the county did not hold it. `E10000021` was **not** added to `la_code_lookup` — one predecessor with two successors doubles on any join by predecessor, so clearing the orphan that way would have created a duplication. Instead `la_s114_notices` gains `attribution` (`direct` / `predecessor`), `successor_codes` (reference only, never a join path) and `attribution_note`, with a CHECK constraint enforcing that a predecessor row cannot exist without both.
- **Gate 14 is narrower, not weaker.** An unresolved code must now either resolve or explain itself through a positive row-level declaration; an orphan that declares nothing still fails, and accepted predecessors are named in the gate output. Proved by injection: an undeclared orphan exits 1, removing it exits 0.
- **A stale known-red entry suppresses the next real failure of its gate.** Found while proving gate 14: the injected failure was absorbed as known-red and the suite exited 2 instead of 1. Entries must be removed in the same change that turns the gate green — recorded in `KNOWN_RED.md` and in the JSON comment.
- **`support_needs_total` held the wrong A3 column for 2023Q2–2024Q4 and is corrected in place.** It carried "households with one support need" instead of "one or more", running at 45–47% of the correct total and matching the adjacent `hh_one_support_need` on 148–164 of 296 authorities. Arbitrated by S1b, an independent extraction of the same sheet, which agrees with a fresh extraction 296/296 in every quarter while the stored value agreed on 1–3. **Misalignment, not revision** — the same A3 defect that quarantined the five `*_suspect` columns, which this one escaped because it read as an aggregate rather than a category. 2,072 rows updated from the verified extraction rather than by reloading data already extracted; all eleven quarters now agree with S1b on 296/296. England totals move e.g. 2024Q4 21,623 → 48,467. Never published — absent from `staging_la_signals`. `reproduction_diff_cells` recomputed: the seven quarters drop from 1,038–1,128 to 790–863, which is the A1 revision alone.
- **Escaped-quarantine sweep across every column in `la_statutory_homelessness`**, run as a data test: for each stored column, does any *other* source column match it better than its own name? Every column best-matches itself, so nothing else escaped. A3 is fully accounted for — five quarantined, one corrected, none remaining. S1b's own aggregates were checked against their parts across all eleven quarters: net difference 0, max absolute difference 0.
- The diagnostic worth reusing: **rate is a signature, not just a magnitude.** A divergence hitting 99% of authorities on one column while hitting 75% on the others is not a revision — revisions touch the authorities that resubmitted, misalignment touches everyone.
